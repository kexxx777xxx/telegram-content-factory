...
**Канал мовчить.** Слот пости не пропускають — його просто нема кому зайняти. Дивитись глибину
черги проєкту (вкладка «Пости»): якщо в ній нема нічого, крім тем, значить не працює генерація —
далі в `rate_limit_state` і в журнал проєкту. Якщо `posts_buffer = 0`, це JIT-режим: пост пишеться
в момент слоту і залежить від моделі саме тоді; надійніше тримати буфер ≥ 1.
...
| `test/load.test.ts` | 50 проєктів на одному розкладі з підробним провайдером і керованим 429 |
...
- планування 50 проєктів ідемпотентне: другий захід не додає в чергу нічого;
