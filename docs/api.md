...
Лог поста: що пішло в модель (`request`), що вона повернула (`response`) і метадані про
зображення (`note`). Пишеться, лише коли ввімкнено відповідний перемикач проєкту
(`logRequests` / `logResponses`); помилки моделі пишуться, якщо ввімкнено будь-який із двох.
