...
import { BATCH_DEADLINE_MS, BATCH_TURNAROUND_MS } from '../src/ai/batch.js';
...
describe('batch tier', () => {
  it('не тримає замовлення довше за стелю вендора', () => {
    // Порогів «скільки лишилось до слоту» більше немає: слотів у постів немає
    // теж (ADR 0009). Лишився один запобіжник — не чекати довше, ніж вендор
    // взагалі обіцяє відповісти.
    expect(BATCH_DEADLINE_MS).toBeLessThanOrEqual(BATCH_TURNAROUND_MS);
    expect(BATCH_DEADLINE_MS).toBeGreaterThanOrEqual(2 * 3600_000);
  });
});
