...
/**
 * Скільки часу до слоту потрібно, щоб узагалі пробувати batch.
 *
 * Було 26 годин — вендорська стеля плюс запас. Виходило правило, яке ніколи не
 * спрацьовувало: буфер планує рівно `postsBuffer` слотів уперед, і на щогодинному
 * розкладі з буфером 20 найдальший слот завжди ближче ніж за добу. Тобто batch
 * для тексту поста був недосяжним за побудовою, хоч проєкт і стояв у
 * «лише batch».
 *
 * 24 години — стеля вендора, а не його звичайний час: наші відповіді приходять
 * за 15 хвилин. Ризик повільного batch несе не цей поріг, а дедлайн: якщо
 * відповідь не встигла до `слот − 2 год`, замовлення скасовується і текст
 * генерується звичайним викликом. Тож тут має стояти не стеля очікування, а
 * мінімальне вікно, у якому чекати ще має сенс: дві години на саме очікування
 * плюс дві, які дедлайн лишає ілюстрації.
 */
export const BATCH_MIN_SLACK_MS = 4 * 3600_000;

/**
 * Наскільки раніше за слот батч перестає бути корисним.
 *
 * Після тексту ще малюється ілюстрація, тож чекати до останньої хвилини не
 * можна. Це і є справжній запобіжник повільного batch: не поріг очікування, а
 * момент, коли замовлення скасовується і текст іде звичайним викликом.
 */
